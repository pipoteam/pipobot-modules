...
class CmdDette(SyncModule):
    ...
    def __init__(self, bot):
        desc = {"": u"Gestionnaire de dettes",
                "add": u"""dette add [name1] [amount] [name2] [reason] : Ajoute une dette de [amount] que doit"""
                u""" payer [name1] à [name2] à cause de [reason]
dette add [name1] [name2] [amount] [name3] [reason] : [name1] et [name2] doivent tous [amount] à [name3]"""
                u""" à cause de [reason]""",
                "multiple": u"dette multiple [name1] [name2] [amount] [name3] [reason] : [name1] et [name2]"
                u" doivent se partager la dette [amount] à payer à [name3] à cause de [reason]",
                "list": u"dette list [name] : Liste les dettes de [name1]",
                "remove": u"dette remove [id1], [id2], [id3] : Supprime les dettes dont les id sont"
                "[id1], [id2], [id3]"
                }
    ...
    def list(self, sender, name1=None, name2=None):
        if name1 is None:
            ...
            if name2 is None:
                ...
                debt_list = self.bot.session.query(Dette, Dette.amount) \
                    .filter(and_(Dette.debtor == name1, Dette.creditor == name2)).all()
                credit_list = self.bot.session.query(Dette, Dette.amount) \
                    .filter(and_(Dette.creditor == name1, Dette.debtor == name2)).all()
            n = 0
            p = 0
            res_debt = ""
            res_credit = ""
    ...
    @answercmd(r"(?P<dtype>add|multiple) (?P<debtor>\S+(?: \S+)*) "
               r"(?P<amount>(?:\d+)(?:\.\d+)?) (?P<creditor>\S+) (?P<reason>.*)")
        ...
        for elt in debtor:
            if elt != creditor:
                ...
                if n is not None and p is not None and (n - p) == 0:
                    deleted = self.bot.session.query(Dette).filter(or_(
                        and_(Dette.debtor == elt, Dette.creditor == creditor),
                        and_(Dette.debtor == creditor, Dette.creditor == elt)
                    )).all()
                    for e in deleted:
