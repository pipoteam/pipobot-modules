...
pendu played : affiche la liste des lettres déjà jouées"""
    ...
    def init(self, sender, word=""):
        if self.pendu.word != "":
            return "Il y a déjà une partie en cours… tu peux la finir, ou au pire 'reset' si tppt"

        if word == "":
            word_list = os.path.join(os.path.dirname(__file__), "wordlist.cfg")
            self.pendu.word = self.pendu.create_word(word_list)
            return "Et c'est parti pour un pendu ! On cherche un mot de %s caractères" % len(self.pendu.word)
        else:
            word = word.strip().lower()
            if all([letter in VALID_CAR for letter in word]):
                self.pendu.word = word
                self.bot.say("Et c'est parti pour un pendu ! On cherche un mot de %s caractères" % len(self.pendu.word))
            else:
                return "Le mot choisi n'est pas valide ! (caractères acceptés : %s)" % VALID_CAR

    @answercmd("reset")
    def reset(self, sender):
        self.pendu.word = ""
        return "Reset effectué, plus qu'à utiliser init pour lancer une nouvelle partie"
...
class PenduTest(ModuleTest):
    def test_pendu_success(self):
        ...
        self.assertEqual(self.bot_answer("!pendu reset"),
                         "Reset effectué, plus qu'à utiliser init pour lancer une nouvelle partie")
        self.assertEqual(self.bot_answer("!pendu played"),
                         "Euh, il faudrait lancer une partie…")
        ...
        for letter, state in ok_tests:
            ...
            self.assertEqual(bot_rep, "Bien vu ! Mot actuel: %s" % state)
